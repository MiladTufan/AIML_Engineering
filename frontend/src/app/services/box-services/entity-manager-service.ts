...
import {
  BlockObject,
  BoxDimensions,
} from '../../models/box-models/BlockObject';
...
export class EntityManagerService {
  ...
  private checkXBounds(box: BlockObject, container: DOMRect) {
    const width =
      box.BoxDims.resizedWidth != 0
        ? box.BoxDims.resizedWidth
        : box.BoxDims.width;
  ...
  private checkYBounds(box: BlockObject, container: DOMRect) {
    const height =
      box.BoxDims.resizedHeight != 0
        ? box.BoxDims.resizedHeight
        : box.BoxDims.height;
  ...
  private getIndexOfCurrentFocusBox(id: number = -99) {
    ...
    let ret = -1;
    if (box !== undefined) ret = this.blockObjects.indexOf(box!);
    return ret;
  }
  ...
  public executeMove(
    ...
    if (parentContainer == null)
      return {
        error: Constants.ERROR_NO_PARENT_CONTAINER,
        success: false,
        obj: obj,
    ...
    const pageContainerRect = (
      pageNew!.htmlContainer! as HTMLElement
    ).getBoundingClientRect();
    ...
    if (commonBoxRef && obj instanceof TextBox) {
      const width =
        obj.BoxDims.resizedWidth === 0
          ? obj.BoxDims.width
          : obj.BoxDims.resizedWidth;
      commonBoxRef.instance.childRefAddOn!.instance.translateX =
        obj.BoxDims.left + width + 10;
      commonBoxRef.instance.childRefAddOn!.instance.translateY =
        obj.BoxDims.top + 10;
    }
  ...
  public rescaleObjOnRender(obj: BlockObject, scale: number) {
    ...
    const condition =
      obj.BoxDims.resizedHeight != 0 || obj.BoxDims.resizedWidth != 0;
    if (condition) {
