...
export class BoxCreationService {
  ...
  constructor() {
    this.eventBusService
      .on(Constants.EVENT_ENTITY_MANAGER_EMIT)
      .subscribe((data: any) => {
        this.eventBusService
          .on(Constants.EVENT_ASSIGN_AND_CREATE_NEW_OBJ, data.obj.id.toString())
          .subscribe((data: any) => {
            this.assignAndCreateNewObj(
              data.obj,
  ...
  private placeTextBoxOntoCanvas(
    ...
    textStyleBlock.instance.translateX =
      textBox.BoxDims.left + textBox.BoxDims.width + 10;
   ...
   * @returns
   */
  private calculateInitialBoxDims(
    mouseX: number,
    mouseY: number,
    scale: number,
   ...
   * @returns
   */
  public createBlockObjectAndInitDims(
    pageNumber: number,
    mouseX: number,
    mouseY: number,
    scale: number,
    ...
    if (!rerender) this.setObjCoords(blockObj, boxDims);
